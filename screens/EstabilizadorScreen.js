...
const EstabilizadorScreen = () => {
    ...
    const categories = ["marca", "modelo", "padrao", "plug", "tipo", "status", "local"];
    ...
    const fetchAllProducts = async () => {
        try {
            const querySnapshot = await getDocs(collection(db, "estabilizadores"));
            const estabilizadores = querySnapshot.docs.map((doc) => ({
                id: doc.id,
                ...doc.data(),
            }));

            setAllProducts(estabilizadores); // Store all products
            setFilteredProducts(estabilizadores); // Initially, show all products
        } catch (error) {
    ...
    const addProduct = async () => {
        ...
        try {
            await setDoc(doc(db, "estabilizadores", patrimonio), {
                marca,
                status,
                plug,
                padrao,
            ...
            setFilteredProducts(prevProducts => [
                ...prevProducts,
                { id: patrimonio, marca, status, local, patrimonio, modelo, plug, padrao, tipo },
            ]);
    ...
    const updateProduct = async () => {
        ...
        try {
            ...
            // Update Firestore
            const docRef = doc(db, "estabilizadores", selectedPatrimonio);
                ...
                renderItem={({ item }) => (
                    <View style={styles.card}>
                        <View>
                            ...
                            <Text style={styles.item}>Tipo: {item.tipo}</Text>
                            <Text style={styles.item}>Marca: {item.marca}</Text>
                            <Text style={styles.item}>Modelo: {item.modelo}</Text>
                            <Text style={styles.item}>Padrão: {item.padrao}</Text>
                            <Text style={styles.item}>Plug: {item.plug}</Text>
                        ...
                        >
                            ...
                            >
                                <View style={styles.modalContainer}>
                                    ...
                                    <TextInput style={styles.inputAdd} placeholder="Tipo (Estabilizador ou nobreak)" placeholderTextColor="gray" value={tipo} onChangeText={setTipo} />
                                    ...
                                    <TextInput style={styles.inputAdd} placeholder="Plug" placeholderTextColor="gray" value={plug} onChangeText={setPlug} />
                                    <TextInput style={styles.inputAdd} placeholder="Padrão das tomadas" placeholderTextColor="gray" value={padrao} onChangeText={setPadrao} />
...
export default EstabilizadorScreen;