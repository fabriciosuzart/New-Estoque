...
const PerifericosScreen = () => {
  ...
  const fetchAllProducts = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "perifericos"));
      const perifericos = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));

      setAllProducts(perifericos); // Store all products
      setFilteredProducts(perifericos); // Initially, show all products
    } catch (error) {
  ...
  const addProduct = async () => {
    ...
    try {
      await setDoc(doc(db, "perifericos", patrimonio), {
        marca,
        status,
        modelo,
        local,
  ...
  const updateProduct = async () => {
    ...
    try {
      ...
      // Update Firestore
      const docRef = doc(db, "perifericos", selectedPatrimonio);
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <View>
              ...
              <TouchableOpacity>
                <AntDesign name="pluscircleo" size={20} color="black" style={styles.buttonAddPeriferico}/>
              </TouchableOpacity>
              <TouchableOpacity>
                <AntDesign name="minuscircleo" size={20} color="black" style={styles.buttonMinusPeriferico}/>
              </TouchableOpacity>
...
export default PerifericosScreen;